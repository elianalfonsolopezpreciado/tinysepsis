"""Multi-seed comparison of the lightweight attention encoder (Tier 2 item
4 of regulatory/model_improvement_roadmap.md) against the GRU (TinySepsis),
on the same 5-seed protocol scripts/run_multiseed.py used for the GRU and
scripts/run_multiseed_cde.py used for the CDE, so all three encoders are
directly comparable.

Requires results/tables/multiseed_raw.csv (GRU "tinysepsis" seed rows) to
already exist -- this script only trains the attention model.
"""
import json
import subprocess
import sys
...
def main():
    for seed in SEEDS:
        tag = f"tinysepsis_attn_seed{seed}"
        ...
        if test_path.exists() and ext_path.exists():
            ...
            continue
        run([PY, str(ROOT / "scripts" / "train_model_attn.py"), "--tag", tag, "--seed", str(seed)], tag)
    ...
    attn_df = pl.DataFrame(rows)
    TABLE_DIR.mkdir(parents=True, exist_ok=True)
    attn_df.write_csv(TABLE_DIR / "multiseed_attn_raw.csv")
    print(attn_df, flush=True)
    ...
    combined = pl.concat([gru_df, attn_df.select(gru_df.columns)])
    combined.write_csv(TABLE_DIR / "multiseed_attn_vs_gru_raw.csv")
    ...
    summary.write_csv(TABLE_DIR / "multiseed_attn_vs_gru_summary.csv")
    print(summary, flush=True)

    gru_gap = gru_df["gap"].to_numpy()
    attn_gap = attn_df["gap"].to_numpy()
    gru_ext = gru_df["external_auroc"].to_numpy()
    attn_ext = attn_df["external_auroc"].to_numpy()

    if len(gru_gap) == len(attn_gap) and len(gru_gap) > 0:
        t_gap, p_gap = stats.ttest_ind(attn_gap, gru_gap, equal_var=False)
        u_gap, pmw_gap = stats.mannwhitneyu(attn_gap, gru_gap, alternative="less")
        t_ext, p_ext = stats.ttest_ind(attn_ext, gru_ext, equal_var=False)
        u_ext, pmw_ext = stats.mannwhitneyu(attn_ext, gru_ext, alternative="greater")
        stats_out = {
            "gap": {"welch_t": float(t_gap), "welch_p": float(p_gap),
                    "mannwhitney_u": float(u_gap), "mannwhitney_p": float(pmw_gap),
                    "cohens_d": cohens_d(attn_gap, gru_gap)},
            "external_auroc": {"welch_t": float(t_ext), "welch_p": float(p_ext),
                                "mannwhitney_u": float(u_ext), "mannwhitney_p": float(pmw_ext),
                                "cohens_d": cohens_d(attn_ext, gru_ext)},
        }
        with open(TABLE_DIR / "multiseed_attn_vs_gru_stats.json", "w") as f:
            json.dump(stats_out, f, indent=2)
        print(json.dumps(stats_out, indent=2), flush=True)

    print("Attention multi-seed comparison complete.", flush=True)
